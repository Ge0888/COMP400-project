...
##### --- CHANGES ---
def train_epoch(args, model, criterion, optimizer, loader, device, dtype, scaler=None, start_epoch=None, end_epoch=None, start_batch=None, end_batch=None, epoch_idx=0):
    if hasattr(model, 'train'):
        model.train()
    ...
    for inp, target in loader:
        ...
        end_batch.record()
        end_batch.synchronize()
        batch_ms = start_batch.elapsed_time(end_batch)

        print(f"batch_time: epoch={epoch_idx} batch={batch_idx} ms={batch_ms}", flush=True)
    
    end_epoch.record()
    end_epoch.synchronize()
    epoch_ms = start_epoch.elapsed_time(end_epoch)

    print(f"epoch_time: epoch={epoch_idx} ms={epoch_ms}", flush=True)
